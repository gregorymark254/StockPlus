...
const Payments = () => {
  ...
  const getUsers = useCallback(async (offset, limit, search) => {
    try {
      const response = await axios.get(`/api/payments?offset=${offset}&limit=${limit}&search=${search}`);
  ...
  useEffect(() => {
    const timerId = setTimeout(() => {
      setLoading(true);
      getUsers((currentPage - 1) * recordsPerPage, recordsPerPage, searchPayment);
    }, 500);
    return () => clearTimeout(timerId);
  }, [currentPage, recordsPerPage, searchPayment, getUsers]);

  return (
    <div className='mx-auto p-4'>
      <div className='bg-white rounded-lg p-4 lg:w-[78vw] xl:w-[81vw] 2xl:w-full'>
        <div className='flex flex-wrap items-center justify-between py-3'>
          <h5 className='text-[#6571ff]'>Showing {payments.length} out of {total} payments</h5>
          <div className='py-2'>
            <form>
